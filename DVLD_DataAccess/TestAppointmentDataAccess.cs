    ...
    public static class TestAppointmentDataAccess {
        ...
        private static DataView _GetPreferredTestHistoryView_Generic(int personID, byte testTypeID, string columnName) {
            string query = $@"SELECT 
                                ...
                                LicenseClasses.{columnName}, 
                                TestAppointments.AppointmentDate,
                                TotalFeesPaid = TestAppointments.FeesPaid + ISNULL(retakeApps.FeesPaid, 0), 
                                TestAppointments.IsLocked FROM TestAppointments LEFT JOIN Applications retakeApps 
                            ON TestAppointments.RetakeFailedTestApplicationID = retakeApps.ApplicationID INNER JOIN LocalDrivingLicenseApplications 
                            ON TestAppointments.LocalDrivingLicenseApplicationID = LocalDrivingLicenseApplications.LocalDrivingLicenseApplicationID INNER JOIN LicenseClasses
                            ON LocalDrivingLicenseApplications.LicenseClassID = LicenseClasses.LicenseClassID INNER JOIN Applications apps
                            ON LocalDrivingLicenseApplications.ApplicationID = apps.ApplicationID
