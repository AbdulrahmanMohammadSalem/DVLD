...
namespace DVLD_DataAccess {
    public static class UserDataAccess {
        ...
        public static bool FindUserById(int userID, ref int personID, ref string username, ref string hashedPassword, ref bool isActive, ref int permissionsID) {
                ...
                try {
                    ...
                    using (SqlDataReader _reader = _command.ExecuteReader()) {
                        if (_reader.Read()) {
                            ...
                            hashedPassword = _reader["HashedPassword"] as string;
        ...
        public static bool FindUserByUsernameAndPassword(ref string username, ref string hashedPassword, ref int userID, ref int personID, ref bool isActive, ref int permissionsID) {
            const string QUERY = "SELECT TOP 1 * FROM Users WHERE Username = @Username AND HashedPassword = @HashedPassword";

            using (SqlConnection _connection = new SqlConnection(DataAccessSettings.ConnectionString))
            using (SqlCommand _command = new SqlCommand(QUERY, _connection)) {
                _command.Parameters.AddWithValue("Username", username);
                _command.Parameters.AddWithValue("HashedPassword", hashedPassword);
        ...
        public static int AddNewUser(int personID, string username, string hashedPassword, bool isActive, int permissionsID) {
            const string QUERY = "INSERT INTO Users (PersonID, Username, HashedPassword, IsActive, PermissionsID) VALUES " +
                                    "(@PersonID, @Username, @HashedPassword, @IsActive, @PermissionsID)";
        ...
        public static bool UpdateUser(int userID, int personID, string username, string hashedPassword, bool isActive, int permissionsID) {
            const string QUERY = "UPDATE Users " +
                                "SET PersonID = @PersonID, Username = @Username, HashedPassword = @HashedPassword, IsActive = @IsActive, PermissionsID = @PermissionsID " +
                                "WHERE UserID = @UserID";
        ...
        public static bool UpdateUserPassword(int userId, ref string newHashedPassword) {
            const string QUERY = "UPDATE Users SET HashedPassword = @NewHashedPassword WHERE UserID = @UserID";

            using (SqlConnection _connection = new SqlConnection(DataAccessSettings.ConnectionString))
            using (SqlCommand _command = new SqlCommand(QUERY, _connection)) {
                _command.Parameters.AddWithValue("UserID", userId);
                _command.Parameters.AddWithValue("NewHashedPassword", newHashedPassword);
