...
namespace DVLD_Business {
    public static class BusinessUtils {
        ...
        public static class RegistryUtils {
            private const string CredentialsKeyPath = @"HKEY_CURRENT_USER\Software\AbdulrahmanMohammadSalem\DVLD";

            public static void ClearLoginCredentials() {
                try {
                    Registry.CurrentUser.DeleteSubKey(@"Software\AbdulrahmanMohammadSalem\DVLD");
                } catch (Exception ex) {
                    EventLogger.LogError(ex);
                }
            }

            public static (string Username, string Password)? RetrieveLoginCredentials() {
                try {
                    var _username = (string) Registry.GetValue(CredentialsKeyPath, "RememberedUsername", null);
                    var _password = (string) Registry.GetValue(CredentialsKeyPath, "RememberedPassword", null);

                    return _username == null ? null : new ValueTuple<string, string>?((_username, _password));
                } catch (Exception ex) {
                    EventLogger.LogError(ex);
                    return null;
                }
            ...
            public static void SaveLoginCredentials(string username, string password) {
                try {
                    Registry.SetValue(CredentialsKeyPath, "RememberedUsername", username, RegistryValueKind.String);
                    Registry.SetValue(CredentialsKeyPath, "RememberedPassword", password, RegistryValueKind.String);
                } catch (Exception ex) {
                    EventLogger.LogError(ex);
                }
            }
