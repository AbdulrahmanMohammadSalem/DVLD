...
namespace DVLD_Business {
    public sealed class User {
        ...
        private User(int userID, int personID, ref string username, ref string hashedPassword, bool isActive, int permissionsID) {
            UserID = userID;
            PersonID = personID;
            Username = username;
        ...
        public string HashedPassword { get; set; } = null; //Default value
        ...
        public static User Find(int userID) {
            ...
            string username = string.Empty, hashedPassword = string.Empty;
            bool isActive = false;

            if (UserDataAccess.FindUserById(userID, ref personID, ref username, ref hashedPassword, ref isActive, ref permissions))
                return new User(userID, personID, ref username, ref hashedPassword, isActive, permissions);
        ...
        private bool _AddNewUser() =>
            (UserID = UserDataAccess.AddNewUser(PersonID, Username, HashedPassword, IsActive, PermissionsID)) != -1;

        private bool _UpdateUser() => UserDataAccess.UpdateUser(UserID, PersonID, Username, HashedPassword, IsActive, PermissionsID);
        ...
        public static User Find(string username, string hashedPassword) {
            int userID = -1, personID = -1, permissions = 0;
            bool isActive = false;

            if (UserDataAccess.FindUserByUsernameAndPassword(ref username, ref hashedPassword, ref userID, ref personID, ref isActive, ref permissions))
                return new User(userID, personID, ref username, ref hashedPassword, isActive, permissions);
        ...
        public bool UpdatePassword(string newHashedPassword) {
            if (_ActiveMode == Modes.AddNew)
                throw new InvalidOperationException("This User instance doesn't have an associated record in the database. It is in the AddNew mode.");

            if (UserDataAccess.UpdateUserPassword(UserID, ref newHashedPassword)) {
