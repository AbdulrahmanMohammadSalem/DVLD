    ...
    public partial class Layout_ManageLocalDrivingLicenseApps : ClosableLayout, IDataConsumer, IEnforcePermissions {
        ...
        private void dgv_localDrivingLicenseApps_CellFormatting(object sender, DataGridViewCellFormattingEventArgs e) {
            ...
            if (dgv_localDrivingLicenseApps.Columns[e.ColumnIndex].ValueType == typeof(DateTime)) {
                ...
                e.Value = $"{e.Value}/3";
                e.FormattingApplied = true; //For some reason I managed to run the application without this line, but couldn't in other cases
            }
        }
        ...
        private void contextMenuStrip1_Opening(object sender, CancelEventArgs e) {
            ...
            if (scheduleTestToolStripMenuItem.Enabled) {
                visionTestToolStripMenuItem.Enabled = _selectedNumPassedTests == 0 && 
                    (   _currentPermissions.Tests_TestAppoints.HasFlag(Tests_TestAppointsManagement.Vision_SeeAllAppointments) || 
                        _currentPermissions.Tests_TestAppoints.HasFlag(Tests_TestAppointsManagement.Vision_CreateNewAppointment) ||
                        _currentPermissions.Tests_TestAppoints.HasFlag(Tests_TestAppointsManagement.Vision_UpdateAppointmentInfo) ||
                        _currentPermissions.Tests_TestAppoints.HasFlag(Tests_TestAppointsManagement.Vision_TakeTest) || 
                        _currentPermissions.Tests_TestAppoints.HasFlag(Tests_TestAppointsManagement.Vision_ExportAppointmentsdata)
                    ) && GlobalFields.ActiveUserPermissions.LocalLicenseApps.HasFlag(LocalLicenseAppsManagement.ViewFullAppDetails);

                writtenTestToolStripMenuItem.Enabled = _selectedNumPassedTests == 1 && 
                    (   _currentPermissions.Tests_TestAppoints.HasFlag(Tests_TestAppointsManagement.Written_SeeAllAppointments) || 
                        _currentPermissions.Tests_TestAppoints.HasFlag(Tests_TestAppointsManagement.Written_CreateNewAppointment) || 
                        _currentPermissions.Tests_TestAppoints.HasFlag(Tests_TestAppointsManagement.Written_UpdateAppointmentInfo) || 
                        _currentPermissions.Tests_TestAppoints.HasFlag(Tests_TestAppointsManagement.Written_TakeTest) || 
                        _currentPermissions.Tests_TestAppoints.HasFlag(Tests_TestAppointsManagement.Written_ExportAppointmentsdata)
                    ) && GlobalFields.ActiveUserPermissions.LocalLicenseApps.HasFlag(LocalLicenseAppsManagement.ViewFullAppDetails);

                drivingTestToolStripMenuItem.Enabled = _selectedNumPassedTests == 2 && 
                    (   _currentPermissions.Tests_TestAppoints.HasFlag(Tests_TestAppointsManagement.Driving_SeeAllAppointments) || 
                        _currentPermissions.Tests_TestAppoints.HasFlag(Tests_TestAppointsManagement.Driving_CreateNewAppointment) || 
                        _currentPermissions.Tests_TestAppoints.HasFlag(Tests_TestAppointsManagement.Driving_UpdateAppointmentInfo) || 
                        _currentPermissions.Tests_TestAppoints.HasFlag(Tests_TestAppointsManagement.Driving_TakeTest) || 
                        _currentPermissions.Tests_TestAppoints.HasFlag(Tests_TestAppointsManagement.Driving_ExportAppointmentsdata)
                    ) && GlobalFields.ActiveUserPermissions.LocalLicenseApps.HasFlag(LocalLicenseAppsManagement.ViewFullAppDetails);
